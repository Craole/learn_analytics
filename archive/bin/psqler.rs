...
/// performs the corresponding actions based on the configuration.
    ...
    match cmd_output {
        Ok(status) => {
            if status.success() {
                ...
                // For example, you can interact with the database using the `diesel` crate or execute SQL queries using the `postgres` crate.
            } else {
